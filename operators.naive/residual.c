...
void __box_residual(box_type *box, int res_id, int phi_id, int rhs_id, double a, double b, double h){
  ...
  double * __restrict__ temp   = box->grids[__temp  ] + ghosts*plane + ghosts*pencil + ghosts;
  ...
  // form helmholtz(phi) = (a alpha I - laplacian)phi
  #pragma omp parallel for private(k,j,i)
  for(k=0;k<box->dim.k;k++){
  for(j=0;j<box->dim.j;j++){
  for(i=0;i<box->dim.i;i++){
        int ijk = i + j*pencil + k*plane;
        temp[ijk] =  a*alpha[ijk]*phi[ijk] - temp[ijk]; // helmholtz = a alphi I - helmholtz
  }}}
  // calculate residual: res = rhs - helmholtz
  #pragma omp parallel for private(k,j,i)
  for(k=0;k<box->dim.k;k++){
  for(j=0;j<box->dim.j;j++){
  for(i=0;i<box->dim.i;i++){
        int ijk = i + j*pencil + k*plane;
        res[ijk] = rhs[ijk] - temp[ijk];
  }}}
}
